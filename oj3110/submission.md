...
```text
Input: รับรหัสต้นทาง รหัสปลายทางและน้ำหนักพัสดุ
output: ถ้าเส้นทางถูกต้อง ต้องแสดงค่าจัดส่งทั้งหมด 2 ตำแหน่ง
ถ้าเส้นทางไม่ถูกต้อง ต้องแสดง Error
```
...
```text
Step 1: รับรหัสต้นทาง รหัสปลายทาง และน้ำหนักของพัสดุ
Step 2: ตรวจสอบว่าเส้นทางที่ส่งตรงกับเส้นทางที่กำหนดไว้หรือไม่
Step 3: ถ้าเป็นเส้นทางที่กำหนด จะคำนวณค่าจัดส่งจาก ค่าบริการเริ่มต้น + น้ำหนัก × ค่าบริการต่อน้ำหนัก
Step 4: ถ้าเส้นทางไม่ตรงกับเงื่อนไขที่กำหนด ให้แสดงคำว่า "Error"
Step 5: ถ้าคำนวณค่าจัดส่งสำเร็จ ให้แสดงราคาโดยมีทศนิยม 2 ตำแหน่ง
```
...
```text
ใช้ทดสอบเส้นทางที่กำหนดไว้และตรวจสอบว่าการคำนวณค่าบริการทำงานถูกต้อง
```
...
```text
ช้ทดสอบว่ารองรับน้ำหนักที่เป็นทศนิยม และเลือกอัตราค่าบริการของเส้นทางได้ถูกต้อง
```
...
```text
ใช้ทดสอบกรณีที่เส้นทางไม่ได้อยู่ในเงื่อนไขที่โปรแกรมกำหนด เพื่อดูว่าโปรแกรมสามารถแจ้งข้อผิดพลาดได้ถูกต้อง
```
