...
```text
Input:
โปรแกรมรับจำนวนเต็ม 2 ค่า X และ Y ในบรรทัดเดียว
โดย X คือระยะกระโดดครั้งแรก และ Y คือระยะเป้าหมาย
Output:
โปรแกรมควรพิมพ์จำนวนครั้งที่กบน้อยต้องกระโดดอย่างน้อย
เพื่อให้ระยะทางรวม >= Y
ถ้าไม่สามารถไปถึงเป้าหมายได้ ให้พิมพ์ -1
```
...
```text
Step 1: อ่านจำนวนเต็ม X และ Y
Step 2: ให้ระยะการกระโดดครั้งแรกเท่ากับ X
Step 3: บวกระยะการกระโดดแต่ละครั้งเข้ากับระยะทางรวม
Step 4: ตรวจว่าระยะทางรวม >= Y หรือไม่
Step 5: ถ้าจริง ให้พิมพ์จำนวนครั้งที่กระโดด
Step 6: ถ้ายังไม่ถึง ให้ลดระยะกระโดดครั้งถัดไปลง 2 เมตร
Step 7: ถ้าระยะกระโดดลดลงจนไม่สามารถไปต่อได้ ให้พิมพ์ -1
```
...
```text
กระโดดได้ 10 → 8 → 6
ระยะรวมคือ 10 + 8 + 6 = 24 ยังไม่ถึง 25
ครั้งที่ 4 ได้ 4 รวมเป็น 28 จึงใช้ 4 ครั้ง
```
...
กระโดดครั้งที่ 1 = 10
ครั้งที่ 2 = 8
รวม 10 + 8 = 18 ถึงเป้าหมายพอดี จึงใช้ 2 ครั้ง
...
```text
กระโดดได้ 5 → 3 → 1
รวมสูงสุด 5 + 3 + 1 = 9 แล้วครั้งต่อไปจะเหลือ -1 ซึ่งกระโดดต่อไม่ได้ จึง ไปไม่ถึง 20 และตอบ -1
